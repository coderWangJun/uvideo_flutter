...
class CompanyEditAllProvider with ChangeNotifier{
  ...
  Widget itemStatusJXWS = Container(
    ...
    child: Align(alignment:Alignment.center,child:Text("继续完善",style: TextStyle(color: Colors.white,fontSize: 12.0),textAlign: TextAlign.center,),
  ));

  Widget getStringStatus(String status){
    return Text(status,style: TextStyle(fontSize: 13.0,color: Color.fromRGBO(153, 153, 153, 1)),);
  }
  ...
  init(){
    DioUtil.request("/company/getCompany").then((value){
      if(DioUtil.checkRequestResult(value)){
        proAll = 0;
        if(value["data"]["logoUrl"] != null&&value["data"]["logoUrl"] != ""){
          url = value["data"]["logoUrl"];
          print("url:$url");
          proAll++;
        }
        if(value["data"]["companyName"] != null&&value["data"]["companyName"] != ""){
          companyName = value["data"]["companyName"];
          proAll++;
        }
        if(value["data"]["introduce"] != null&&value["data"]["introduce"] != ""){
          introduce = value["data"]["introduce"];
          proAll++;
        }
        String completeness = value["data"]["completeness"];
        proValue = double.parse(completeness.split("%")[0]);
        if(value["data"]["address"] != null&&value["data"]["address"] != ""){
          address = value["data"]["address"];
          proAll++;
        }

        if(value["data"]["financingStageName"] != null&&value["data"]["financingStageName"] != ""){
          financingStageNo = value["data"]["financingStage"] as num;
          financingStage = value["data"]["financingStageName"] as String;
          proAll++;
        }

        if(value["data"]["staffScaleName"] != null&&value["data"]["staffScaleName"] != ""){
  ...
  initData(){
    DioUtil.request("/resource/getFinancingStageList",method: 'GET').then((value){
      if(DioUtil.checkRequestResult(value)){
  ...
  changeCompanyName(String value){
    DioUtil.request("/company/updateCompany",parameters: {"companyName":value}).then((value){
      if(DioUtil.checkRequestResult(value)){
        init();
      }
    });
  ...
  changeAddress(String value){
    DioUtil.request("/company/updateCompany",parameters: {"address":value}).then((value){
      if(DioUtil.checkRequestResult(value)){
        init();
      }
    });
  ...
  changeIntroduce(String value){
    DioUtil.request("/company/updateCompany",parameters: {"introduce":value}).then((value){
      if(DioUtil.checkRequestResult(value)){
        init();
      }
    });
  ...
  changeFinancingStage(int value){
    DioUtil.request("/company/updateCompany",parameters: {"financingStageNo":value}).then((value){
      if(DioUtil.checkRequestResult(value)){
        init();
      }
    });
  ...
  changeStaffScaleName(int value){
    DioUtil.request("/company/updateCompany",parameters: {"staffScaleNo":value}).then((value){
      if(DioUtil.checkRequestResult(value)){
        init();
      }
    });
  ...
  uploadLogoImage(File pickFile){
    FormData formData = FormData.fromMap({'file': MultipartFile.fromFileSync(pickFile.path)});
    DioUtil.request("/company/uploadCompanyLogo", parameters: formData).then((responseData) {
      if(DioUtil.checkRequestResult(responseData)){
        ...
        url = data["worksUrl"];
        DioUtil.request("/company/updateCompany",parameters: {"logoUrl":url}).then((value){
          if(DioUtil.checkRequestResult(value)){
            print("ok");
            init();
          }
