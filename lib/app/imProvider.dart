...
class ImProvider with ChangeNotifier{
  ...
  static final String DEF_HEAD_IMAGE_URL = "http://mingyankeji.oss-cn-chengdu.aliyuncs.com/default-head-portrait/%E4%BC%98%E8%A7%86APP%E9%BB%98%E8%AE%A4%E5%A4%B4%E5%83%8F.png1597053811548?Expires=3173853811&OSSAccessKeyId=LTAI4GDRbwvuszWXHCNebT2j&Signature=tIsYLrlR9C01liBA1UtmSriROUI%3D";
...
//          showNotify(list[0].note);
      ...
      if(list[0].elemList[0].nodeType==MessageNodeType.Custom){
        CustomMessageNode customMessageNode = list[0].elemList[0] as CustomMessageNode;
        List<String> mListString = customMessageNode.data.split(",");
        if(mListString[0] == StorageManager.RTC_YAOQING){//收到了新的语音邀请
          //对方的ID，作为接受时的房间号以及发送回调信息的sessionId
          Duration dif = DateTime.now().difference(DateTime.fromMillisecondsSinceEpoch(list[0].timestamp*1000));
          if(dif.inSeconds<15){
            String fromSessionId = list[0].sender;
            if(!flagIsOnRtcChat){//正在空闲，可以接受，设置繁忙，跳转到邀请页面
              flagIsOnRtcChat = true;
              if(mListString[1] == "1"){
                Get.to(VideoRtcWait(fromSessionId));
              }else{
                Get.to(AudioRtcWait(fromSessionId,isSender: false,));
              }
            }else{//当前繁忙，发送繁忙消息回去
              TencentImPlugin.sendMessage(sessionId: fromSessionId, sessionType: SessionType.C2C, node: CustomMessageNode(data:StorageManager.RTC_FANMANG));
            }
          }
        }else if(mListString[0] == StorageManager.RTC_QUXIAO){//对方取消了音视频邀请
          flagIsOnRtcChat = false;
          //退出视频通话房间，显示对方拒绝
          g_eventBus.emit(GlobalEvent.outRtcChat,"对方取消了邀请");
        }else if(mListString[0] == StorageManager.RTC_FANMANG){//对方正在其他音视频中，繁忙
          //退出视频通话房间，显示对方当前繁忙稍后再试
          flagIsOnRtcChat = false;
          g_eventBus.emit(GlobalEvent.outRtcChat,"对方繁忙,请稍后再试");
        }else if(mListString[0] == StorageManager.RTC_JUJUE){//对方拒绝了音视频邀请
          flagIsOnRtcChat = false;
          g_eventBus.emit(GlobalEvent.outRtcChat,"对方拒绝了你的邀请");
        }else if(mListString[0] == StorageManager.RTC_JIESHOU){//对方接受了语音通话
          flagIsOnRtcChat = true;
  ...
  initListChat(){
    ...
    TencentImPlugin.getConversationList().then((value){
      ...
      if(sessionIdList.length!=0){
        DioUtil.request("/user/getUserMSGByTxUserid",parameters: {"ids":sessionIdList}).then((values){
