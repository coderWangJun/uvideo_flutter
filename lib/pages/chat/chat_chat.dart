...
class _ChatChatState extends State<ChatChat> {
  ...
  Widget build(BuildContext context) {
    ...
    if (imProvider.dateMap.length == 0) {
      imProvider.initListChat();
    } //防止初始化app加载列表失败，每次重绘都判断是否存在列表，不存在就尝试刷新
    return BaseView(
...
//        SearchBar('通过姓名或公司 搜索联系人'),
          Expanded(
              child: imProvider.dateMap.length == 0
                  ? EmptyWidget(
                      showTitle: "列表空空如也,快去聊天吧~",
                    )
                  : ListView(children: _getList(model, imProvider))
  ...
  List<Widget> _getList(ChatChatProvider model, ImProvider imProvider) {
    ...
    imProvider.chatList.forEach((e) {
      if (imProvider.dateMap[e.id] != null) {
        ...
        DateTime timedif =
            DateTime.fromMillisecondsSinceEpoch(e.message.timestamp * 1000)
                .toLocal();
        String time = DataTimeToString.toTextString(timedif);
        Widget w = GestureDetector(
            behavior: HitTestBehavior.opaque,
            onTap: () {
              Get.to(ChatRoute(e.id, imProvider.dateMap[e.id]));
            },
            child: Container(
              padding:
                  EdgeInsets.only(left: 20, right: 20, top: 12.5, bottom: 12.5),
              child: Row(
                children: <Widget>[
                  Container(
                    width: 55.0,
                      ...
                      children: <Widget>[
                        ClipOval(
                          child: Container(
                            ...
                            child: Image.network(
                              imProvider.dateMap[e.id]["headPortraitUrl"] ==
                                      null
                        ...
                        Visibility(
                            visible: e.unreadMessageNum > 0 ? true : false,
                            child: Container(
...
//                          _buildJobTitle(rowData),
                                Text(
                                    imProvider.dateMap[e.id]["companyName"] ??=
                                        "",
                                    style: TextStyle(
                                        fontSize: 12,
                        ...
                        Row(
                          children: <Widget>[
                            Text(e.message.peerReaded ? "[已读]" : "[未读]",
                                style: TextStyle(
                                    fontSize: 13,
                                    color: Color.fromRGBO(153, 153, 153, 1))),
                            SizedBox(width: 3),
