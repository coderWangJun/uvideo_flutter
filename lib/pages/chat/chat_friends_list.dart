...
class ChatFriendsListProvide extends ChangeNotifier{
  ...
  void checkAllFriends(){
    if(!mListTitle[0]["open"]){//打开列表逻辑
      mListTitle[0]["open"] = true;
      mListTitle[0]["icon"] = icon_on;
      if(mAllFriendsList.length==0 || mAllFriendsList == null){
        DioUtil.request("/user/getUserFriendsList").then((value){if(DioUtil.checkRequestResult(value)){
          if(value['data']!=null){
  ...
  void checkMyCare(){
    if(!mListTitle[1]["open"]){//打开列表逻辑
      mListTitle[1]["open"] = true;
      mListTitle[1]["icon"] = icon_on;
      if(mMyCarePersons.length==0 || mMyCarePersons == null){
        DioUtil.request("/user/getUserCareList").then((value){if(DioUtil.checkRequestResult(value)){
          if(value['data']!=null){
  ...
  void checkMyFun(){
    if(!mListTitle[2]["open"]){//打开列表逻辑
      mListTitle[2]["open"] = true;
      mListTitle[2]["icon"] = icon_on;
      if(mMyFunPersons.length==0 || mMyFunPersons == null){
        DioUtil.request("/user/getUserFansList").then((value){if(DioUtil.checkRequestResult(value)){
          if(value['data']!=null){
  ...
  changeShowCareStatus(int index,int position){
    switch(index){
      case 0 : mAllFriendsList[position]['showCareText'] = !mAllFriendsList[position]['showCareText'];break;
      case 1 : mMyCarePersons[position]['showCareText'] = !mMyCarePersons[position]['showCareText'];break;
      case 2 : mMyFunPersons[position]['showCareText'] = !mMyFunPersons[position]['showCareText'];break;
    }
    notifyListeners();
  }
...
//              ],
  ...
  Widget _getBody(ChatFriendsListProvide model){
    return SingleChildScrollView(child: Column(
      ...
      children: <Widget>[
        ...
        Visibility(child:Column(children:_getItemsWidget(model, 0)), visible: model.mListTitle[0]['open'],),
        _getTitleWidget(model, 1,"careCount"),
        Visibility(child:Column(children: _getItemsWidget(model, 1)),visible: model.mListTitle[1]['open'],),
        _getTitleWidget(model, 2,"fansCount"),
        Visibility(child: Column(children:_getItemsWidget(model, 2)),visible: model.mListTitle[2]['open'],),
      ],

    ),);
  }

  Widget _getTitleWidget(ChatFriendsListProvide model,int index,String mapName){
    return GestureDetector(behavior: HitTestBehavior.opaque,child: Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          Text(model.mListTitle[index]['title'],style: TextStyle(
          ...
          Row(
            ...
            mainAxisSize: MainAxisSize.min,
            children: <Widget>[
              Text("${model.countMap[mapName]}",style: TextStyle(color: Color.fromRGBO(102, 102, 102, 1),fontSize: 13.0),),
              SizedBox(width: 10.0,),
              Icon(model.mListTitle[index]['icon'])
            ],
          )
        ],
      ),
      ...
      print("oook");
      switch(index){
        case 0 : model.checkAllFriends();break;
        case 1 : model.checkMyCare();break;
        case 2 : model.checkMyFun();break;
      }
    },);
  }
  ...
  List<Widget> _getItemsWidget(ChatFriendsListProvide model,int index){
    List mDataList;
    switch(index){
      case 0 : {mDataList = model.mAllFriendsList;break;}
      case 1 : {mDataList = model.mMyCarePersons;break;}
      case 2 : {mDataList = model.mMyFunPersons;break;}
    }
    int position = -1;
    return mDataList.map((e){position++;int posiNum = position;
      return GestureDetector(behavior: HitTestBehavior.opaque,child: Container(
      ...
      child: Row(
        ...
        children: <Widget>[Row(
          ...
          mainAxisAlignment: MainAxisAlignment.start,
          children: <Widget>[
            Container(width: 50.0,height: 50.0,decoration: BoxDecoration(
              color: Colors.grey,
              image: DecorationImage(image: NetworkImage(
                  index!=2?e['targetUserHeadPortrait']??=ImProvider.DEF_HEAD_IMAGE_URL:e['userHeadPortrait']??=ImProvider.DEF_HEAD_IMAGE_URL),
            ...
            Column(
              ...
              children: <Widget>[
                Text(e['name']??="",style: TextStyle(color: Color.fromRGBO(51, 51, 51, 1),fontSize: 15.0,fontWeight: FontWeight.bold),overflow: TextOverflow.ellipsis,),
//                Text("公司·职位",style: TextStyle(color: Color.fromRGBO(102, 102, 102, 1),fontSize: 13.0),),
              ],
            ),],),

          GestureDetector(child: Container(height: 24.0,constraints:BoxConstraints(minWidth: 20.0),
            padding: EdgeInsets.symmetric(horizontal: 7.0),
            decoration: BoxDecoration(
              border: Border.all(color: Color.fromRGBO(235, 235, 235, 1),width: 1.0),
              borderRadius: BorderRadius.circular(12.0)
            ),child: Center(child:Text(e["showCareText"]?"取消关注":"关注",style: TextStyle(
            ...
            if(e["showCareText"]){
              ...
              DioUtil.request(requestUrl,parameters: {'targetUserid':index!=2?e['targetUserid']:e['userid']}).then((value){
      ...
      ///点击去聊天
      Get.to(UserDetailRoute(userId: index!=2?e['targetUserid']:e['userid'],));
