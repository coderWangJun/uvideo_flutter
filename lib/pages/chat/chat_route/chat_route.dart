...
//    TencentImPlugin.addListener(_listener);
    ...
    DioUtil.request("/user/getUserMSGByTxUserid", parameters: {
      'ids': [toSessionId]
    }).then((value) {
...
//    TencentImPlugin.removeListener(_listener);
  ...
  sendMessage(MessageNodeType type) {
    ...
    if (1 == 0) {
      return;
    } else if (type == MessageNodeType.Text) {
      node = TextMessageNode(content: text);
    } else if (type == MessageNodeType.Image) {
      node = ImageMessageNode(path: text, level: 1);
    ...
    TencentImPlugin.sendMessage(
            sessionId: toSessionId, sessionType: SessionType.C2C, node: node)
        .then((value) {
...
//        scroll.jumpTo(scroll.position.maxScrollExtent);
  ...
  refreshUiIsSend(MessageNodeType type, MessageEntity node) {
    ...
    /// 测试代码区域开始
    // TencentImPlugin.getMessages(
    //         sessionId: toSessionId, sessionType: SessionType.C2C, number: 10)
    //     .then((value) {
    //   print('getMessages============ start');
      ...
      Widget appendUi = Container(
          ...
          child: Row(
            ...
            children: <Widget>[
              //头像先写死了，暂时不知道怎么获取这个东西
              _TextUiWidgetImg(
                  (imageNode.path != null && imageNode.path != "")
                      ? imageNode.path
                      : imageNode.imageData[ImageType.Original].url,
                  true),
              SizedBox(
                width: 10.0,
              ),
          ...
          appendUi = Container(
              ...
              child: Row(
                ...
                children: <Widget>[
                  //头像先写死了，暂时不知道怎么获取这个东西
                  _TextVideoWidget(
                      textNode.data.split(',')[0], true, Icons.videocam),
                  SizedBox(
                    width: 10.0,
                  ),
                  _getHeadImg(myHeadImg),
...
//          _TextUiWidgetSound(node, false, countSound),
          ...
          appendUi = Container(
              ...
              child: Row(
                ...
                children: <Widget>[
                  ...
                  _TextVideoWidget(
                      textNode.data.split(',')[0], false, Icons.videocam),
                ],
              ));
        }
      }
  ...
  Widget _TextVideoWidget(String data, bool flag, IconData iconData) {
    ...
    if (1 == 0) {
      return null;
    } else if (data == StorageManager.RTC_YAOQING) {
      texts = "邀请通话";
    } else if (data == StorageManager.RTC_JUJUE) {
      texts = "拒绝了邀请";
    } else if (data == StorageManager.RTC_FANMANG) {
      texts = "正在其他通话";
    } else if (data == StorageManager.RTC_QUXIAO) {
      texts = "取消了邀请";
    } else if (data == StorageManager.RTC_JIESHOU) {
      texts = "接受了语音邀请";
    } else {
      texts = data;
    }
  ...
  Widget _transferUiWidget(String text, bool flag) {
    return Container(
      ...
      child: Text(
        flag ? "成功转账给对方${text}U币" : "对方转账给你${text}U币,已存入你的余额",
        style: UiUtil.getTextStyle(255, 13.0),
                ...
                children: <Widget>[
                  GestureDetector(
                    ...
                    child: UiUtil.getContainer(
                        30.0,
                        10.0,
                        Color.fromRGBO(79, 154, 247, 1),
                        Text(
                          "同意",
                          style: UiUtil.getTextStyle(0, 13.0, isBold: true),
                        ),
                        mWidth: 70.0),
                    onTap: () {
                      setState(() {
                  ...
                  GestureDetector(
                    ...
                    child: UiUtil.getContainer(
                        ...
                        Text(
                          "拒绝",
                          style: UiUtil.getTextStyle(255, 13.0, isBold: true),
                        ),
                        mWidth: 70.0),
                    onTap: () {
                      setState(() {
...
class SoundWidget extends StatefulWidget {
  ...
  _SoundWidgetState createState() => _SoundWidgetState(
      this.chatDomeProvider, this.node, this.flag, this.index);
}
...
class _SoundWidgetState extends State<SoundWidget>
  ...
  void initState() {
    ...
    _controller = AnimationController(
        vsync: this, duration: Duration(milliseconds: mill));
    animation = new Tween(begin: 56.0, end: 0.0).animate(_controller)
      ..addListener(() {
        setState(() {
...
class _ChatDomeImp extends State<ChatRoute>
...
//    if(_chatDomeProvider.sessionLast==sessionId){
    TencentImPlugin.setRead(sessionId: sessionId, sessionType: SessionType.C2C)
        .then((value) {
...
//         print("----${date.millisecondsSinceEpoch}");
        ...
        model.setLongTime(
            DateFormat('mm:ss:SS', 'en_GB').format(date).substring(0, 5));
      });
    } catch (e) {
      print("throw error----->$e");
    }
  ...
  Widget build(BuildContext context) {
    ...
    return BaseView(
      ...
      onReady: (model) {
        ...
        model.hisHeadImg = widget.hisUserInfo["headPortraitUrl"] ??=
                      ...
                      title: Column(
                        ...
                        children: <Widget>[
                          ...
                          Row(
                            ...
                            children: <Widget>[
                              Text(
                                widget.hisUserInfo["companyName"] ?? "",
                                style: TextStyle(
...
//              )
            ...
            Visibility(
                ...
                child: Container(
                  ...
                  child: Text(
                    "提示框",
                    style: TextStyle(color: Color.fromRGBO(206, 163, 44, 1)),
                  ),
                )),
                  ...
                  children: <Widget>[
                    Row(
                      children: <Widget>[
                        ...
                        IconButton(
                            icon: Image.asset(join(
                                AssetsUtil.assetsDirectoryChat, "vo2586.png")),
                            onPressed: () async {
                              if (await Permission.camera.request().isGranted) {
                                model.text =
                                    StorageManager.RTC_YAOQING + ',' + "1";
                                model.sendMessage(MessageNodeType.Custom);
                                Get.to(VideoRtcChat(
                                    g_accountManager
                                        .currentUser.tXIMUser.txUserid,
                                    imgUrl: model.hisHeadImg,
                                    chatDomeProvider: model));
                              } else {
                                print("权限拒绝");
                              }
                        ...
                        IconButton(
                            icon: Image.asset(join(
                                AssetsUtil.assetsDirectoryChat,
                                "sound2587.png")),
                            onPressed: () async {
...
//              height: 200.0,
                            ...
                            child: _getBottomItem(
                                model, context, model.botItemFlag)))
                  ],
                ))
          ],
        )
  ...
  Widget _getBottomItem(
      ChatDomeProvider model, BuildContext context, int posIndex) {
    if (posIndex == 0) {
      //选项列表
      return Wrap(
        children: <Widget>[
          ...
          GestureDetector(
            ...
            onTap: () {
              Get.to(Transfer({
                'userid': model.hisUserId,
                'headImageUrl': model.hisHeadImg,
                'userName': widget.hisUserInfo["name"] ?? ""
              })).then((value) {
                if (value != null) {
                  // todo----->
...
//                    )
                          ...
                          : BoxDecoration(
                              image: DecorationImage(
                                  image: AssetImage("assets/gif/sound.gif")),
                              borderRadius: BorderRadius.circular(50.0),
                            ),
                      child: model.isSoundOn
                          ? Image.asset(
                              join(AssetsUtil.assetsDirectoryChat, "yyb.png"),
                              color: model.staSound['iconColor'],
                            )
                          : SizedBox.shrink(),
                    ),
                  ),
            ...
            child: Positioned(
              ...
              child: ClipOval(
                child: Container(
                  ...
                  color: model.colorQx,
                  child: Image.asset(
                      join(AssetsUtil.assetsDirectoryChat, "sch.png")),
                ),
              ),
            ),
          )
