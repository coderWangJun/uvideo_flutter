...
class _HomeGridResumeState extends State<HomeGridResume> {
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.model.modelListRes.length>0){
      return RefreshIndicator(child: GridView(
        controller: _scrollController,
        padding: EdgeInsets.only(left: 40.w, right: 40.w, top: 0, bottom: 40.h),
        ...
        children: widget.model.modelListRes.asMap().keys.map((index) {
          ...
          return GestureDetector(
                    ...
                    child: Stack(
                      ...
                      children: <Widget>[
                        ClipRRect(
                          ...
                          child: resumeModel.coverUrl == null ? SizedBox() : CachedNetworkImage(
                            width: gridWidth,
                            ...
                            Row(
                              children: <Widget>[
                                ClipRRect(
                                  ...
                                  child: resumeModel.headPortraitUrl == null ? SizedBox() : CachedNetworkImage(
                                    width: 24,
                                    height: 24,
                                    imageUrl: resumeModel.headPortraitUrl ?? "",
                                    placeholder: (BuildContext context, String url) {
                                      return Image.asset(join(AssetsUtil.assetsDirectoryCommon, "def_avatar.png"), width: 24, height: 24);
                                    },
                                  ),
                                ),
                                SizedBox(width: 5),
                                Text(resumeModel.realname ?? "", overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 17, color: ColorConstants.textColor51, fontWeight: FontWeight.bold)),
                              ],
                            ),
                            Text('${resumeModel.workingYears ?? 0}年', style: TextStyle(fontSize: 13, color: ColorConstants.textColor153))
                          ],
                        ),
                        SizedBox(height: 5),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: <Widget>[
                            Text(resumeModel.title, overflow: TextOverflow.ellipsis, style: TextStyle(color: Color.fromRGBO(102, 102, 102, 1), fontSize: 13)),
                            Expanded(
                              child: Text(resumeModel.salaryTreatmentString ?? "",
                                  maxLines: 1,
                                  textAlign: TextAlign.right,
                                  style: TextStyle(color: ColorConstants.themeColorBlue, fontSize: 15)),
                            )
                          ],
                        )
                      ],
            ...
            onTap: () {
              ...
              Get.to(VideoPlayerHoriContainer(VideoPlayType.resumeVideo, currentResumeModel: resumeModel));
