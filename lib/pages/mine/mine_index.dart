...
class _MineIndexState extends State<MineIndex> with SingleTickerProviderStateMixin {
  bool _hasVideo = true;
              ...
              headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
                return <Widget>[
                  SliverAppBar(
                    ...
                    pinned: true,
                    floating: true,
                    expandedHeight: (g_accountManager.currentUser.typeId == 1) ? 410 : 480,
...
//                  MineMarketList()
                  MarketPostList(postModel, "/market/getMarketList", {"queryType":1},refreshByMine: true,controller: _scrollController,),
                ],
              ),
            ),
            Positioned(
                ...
                child: _hasVideo ? SizedBox.shrink() : Image.asset("assets/gif/publish.gif", width: 168, height: 104)
            ),
          ],
        ),
      )
  ...
  void _refreshData() {
    ...
    int typeId = accountModel.typeId; // 1 个人；2 企业
    if (typeId == 1) {
      AccountPersonModel personModel = accountModel.userData;
      _name = personModel.realname ?? personModel.phonenumber;
      _headImgUrl = personModel.headPortraitUrl ?? ImProvider.DEF_HEAD_IMAGE_URL;
      // 加载我的简历视频
      _loadMyResumeVideos();
    } else if (typeId == 2) {
  ...
  void _checkVideoState() {
    if (_shortVideoList.length > 0) {
      if (g_accountManager.currentUser.typeId == 1 && _resumeVideoList.length > 0) {
        _hasVideo = true;
      } else if (g_accountManager.currentUser.typeId == 2 && _companyVideoList.length > 0) {
        _hasVideo = true;
      } else {
        _hasVideo = false;
      }
            ...
            child: Row(
              children: <Widget>[
                ...
                Container(
                  ...
                  child: Text(AccountManager.instance.ringSwitch ? "关闭求职铃" : "开启求职铃", style: TextStyle(fontSize: 9, color: Colors.white, fontWeight: FontWeight.bold)),
                )
              ],
            ),
            onTap: () async{
              AccountManager.instance.ringSwitch = !AccountManager.instance.ringSwitch;
              g_storageManager.setStorage(StorageManager.RTC_SWITCH, AccountManager.instance.ringSwitch);
              if (AccountManager.instance.ringSwitch){
                if (await Permission.locationWhenInUse
                    .request()
              .isGranted && await Permission.location
                  .request()
                  .isGranted) {
                  debugPrint("我是有权限的");
              }
...
//              ),
              ...
              InkWell(
                child: Container(
                  width: 32,
                  height: 44,
                  child: Image.asset(join(AssetsUtil.assetsDirectoryCommon, 'icon_setting.png')),
                ),
                onTap: () {
                  Get.to(SettingIndex());
                },
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Row(
                      ...
                      children: <Widget>[
                        Text(_name, style: TextStyle(fontSize: 24, color: Colors.white, fontWeight: FontWeight.bold)),
                        SizedBox(width: 5),
                        Text(_job, style: TextStyle(fontSize: 17, color: Colors.white, fontWeight: FontWeight.bold))
                      ],
                    ),
...
//                Image.asset(join(AssetsUtil.assetsDirectoryCommon, 'def_company_avatar.png')),
                ClipOval(child: Container(width: 65.0,height: 65.0,child: Image.network(_headImgUrl,fit: BoxFit.cover,),),),
                onTap: (){
            ...
            children: <Widget>[
              ButtonTheme(
                ...
                child: FlatButton(
                  ...
                  child: Column(
                    children: <Widget>[
                      Text("${_statisticModel.careCount ?? 0}", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: Colors.white)),
                      Text('关注', style: TextStyle(fontSize: 15, color: Colors.white))
                    ],
                  ),
                ),
              ),
              ButtonTheme(
                ...
                child: FlatButton(
                  ...
                  child: Column(
                    children: <Widget>[
                      Text('${_statisticModel.fansCount ?? 0}', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: Colors.white)),
                      Text('粉丝', style: TextStyle(fontSize: 15, color: Colors.white))
                    ],
                  ),
                ),
              ),
              ButtonTheme(
                ...
                child: FlatButton(
                  ...
                  child: Column(
                    children: <Widget>[
                      Text('${_statisticModel.collectCount ?? 0}', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: Colors.white)),
                      Text('收藏', style: TextStyle(fontSize: 15, color: Colors.white))
                    ],
                  ),
                ),
              ),
              ButtonTheme(
                ...
                child: FlatButton(
                  ...
                  child: Column(
                    children: <Widget>[
                      Text('${_statisticModel.likeCount ?? 0}', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: Colors.white)),
                      Text('喜欢', style: TextStyle(fontSize: 15, color: Colors.white))
                    ],
                  ),
                ),
              ),
            ...
            ButtonTheme(
              ...
              child: FlatButton(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Text('${_statisticModel.deliveryCount ?? 0}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: ColorConstants.textColor51)),
                    Text('投递', style: TextStyle(fontSize: 13, color: Color.fromRGBO(101, 101, 101, 1))),
                  ],
                ),
              ),
            ),
            ButtonTheme(
              ...
              child: FlatButton(
                ...
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: <Widget>[
                    Text('${_statisticModel.circleCount ?? 0}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: ColorConstants.textColor51)),
                    Text('圈子', style: TextStyle(fontSize: 13, color: Color.fromRGBO(101, 101, 101, 1))),
                  ],
                ),
              ),
            ),
...
//            ),
            ...
            ButtonTheme(
              ...
              child: FlatButton(
                ...
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: <Widget>[
                    Text('${_statisticModel.assetsScore ?? 0}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: ColorConstants.textColor51)),
                    Text('资产', style: TextStyle(fontSize: 13, color: Color.fromRGBO(101, 101, 101, 1))),
                  ],
                ),
                onPressed: () {
  ...
  Widget _buildOnlineJobRow() {
    return (g_accountManager.currentUser.typeId == 2) ? InkWell(
      child: Container(
        height: 50,
        padding: EdgeInsets.only(left: 15, right: 15),
        ...
        child: Row(
          ...
          children: <Widget>[
            Row(
              children: <Widget>[
                Text('${_statisticModel.onlineJobCount ?? 0}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: ColorConstants.textColor51)),
                Text('个在线职位', style: TextStyle(fontSize: 13, color: ColorConstants.textColor51)),
              ],
            ),
            Row(
              children: <Widget>[
                Text('管理职位', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: ColorConstants.textColor51)),
                SizedBox(width: 5),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          InkWell(
            child: Container(
              ...
              child: Row(
                ...
                crossAxisAlignment: CrossAxisAlignment.center,
                children: <Widget>[
                  Text(g_accountManager.currentUser.typeId == 1 ? "我的简历视频" : "企业宣传片", style: TextStyle(fontSize: 17, color: ColorConstants.textColor51, fontWeight: FontWeight.w500)),
          ...
          hasVideo == false ? InkWell(
            child: Container(
              width: 72,
              height: 80,
              margin: EdgeInsets.only(left: 20),
              child: Image.asset(join(AssetsUtil.assetsDirectoryCommon, "btn_add_video.png"), fit: BoxFit.fill),
            ),
            onTap: () {
              if (g_accountManager.currentUser.typeId == 1) {
            ...
            child: ListView.builder(
              itemCount: g_accountManager.currentUser.typeId == 1 ? _resumeVideoList.length : _companyVideoList.length,
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                if (g_accountManager.currentUser.typeId == 1) {
                  ...
                  CompanyVideoModel companyModel = _companyVideoList[index];
                  ...
                  child: Container(
                      ...
                      child: Stack(
                        ...
                        children: <Widget>[
                          Container(
                            decoration: BoxDecoration(
                              border: Border.all(color: ColorConstants.dividerColor.withOpacity(0.5), width: 1),
                          ...
                          Image.asset(join(AssetsUtil.assetsDirectoryCommon, 'icon_play_on_list.png'))
                        ],
                      )
                  ),
                  onTap: () {
  ...
  Widget _buildTabBar() {
    return PreferredSize(
      ...
      child: Container(
        ...
        width: double.infinity,
        decoration: BoxDecoration(
          border: Border(bottom: BorderSide(color: Color.fromRGBO(237, 237, 237, 1), width: 1))
        ),
        child: TabBar(
          controller: _tabController,
  ...
  void _loadMyResumeVideos() {
    var params = {"queryType": 1};
    DioUtil.request('/resume/getMediaResume', parameters: params).then((responseData) {
      bool success = DioUtil.checkRequestResult(responseData, showToast: false);
      if (success) {
