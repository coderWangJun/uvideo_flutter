...
class ResumeVideoManage extends StatefulWidget {
  @override
  _ResumeVideoManageState createState() => _ResumeVideoManageState();
}

class _ResumeVideoManageState extends State<ResumeVideoManage> {
  bool _editting = false;
  bool _selectAll = false;

  List<HomeResumeModel> _videoModelList = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBarWhite(title: "简历管理", actions: <Widget>[
  ...
  Widget _buildVideoList() {
    ...
    return GridView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        HomeResumeModel videoModel = _videoModelList[index];
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Text("发布于${videoModel.createdTime != null ? videoModel.createdTime.substring(5, 10) : ''}", style: TextStyle(fontSize: 13, color: videoModel.hidden == 2 ? Color.fromRGBO(136, 136, 136, 1).withOpacity(0.5) : Color.fromRGBO(136, 136, 136, 1)))
                    ],
                  ),
                )
              ],
  ...
  Widget _buildButtonWidgets(parentContext) {
    ...
    for (HomeResumeModel videoModel in _videoModelList) {
  ...
  void _batchOpenVideosAction(parentContext) {
    ...
    for (HomeResumeModel companyVideoModel in _videoModelList) {
    ...
    DioUtil.request('/resume/getMediaResume', parameters: params).then((responseData) {
      ...
      if (success) {
        ...
        List<HomeResumeModel> modelList = dataList.map((json) {
          HomeResumeModel resumeModel = HomeResumeModel.fromJson(json);
          resumeModel.selected = false;
          return resumeModel;
        }).toList();
  ...
  void _deleteSelectedVideosRequest(selectedIds) {
    ...
    DioUtil.request("/resume/deleteMediaResumeBatch", parameters: selectedIds).then((responseData) {
