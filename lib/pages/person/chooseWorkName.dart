...
class ChooseWorkNameProvider with ChangeNotifier{
  ...
  init(){
    if(isGangWei){
      DioUtil.request("/resource/getSubPosition",parameters: {"id":0}).then((value){
      ...
      DioUtil.request("/resource/getSubIndustry",parameters: {"id":0}).then((value){
        if(DioUtil.checkRequestResult(value)){
          mListOne[0] = value["data"] as List;
          notifyListeners();
        }
  ...
  Future<bool> getNext(int id) async{
    if(isGangWei){
      var value = await DioUtil.request("/resource/getSubPosition",parameters: {"id":id});
        if(DioUtil.checkRequestResult(value)){
      ...
      var value = await DioUtil.request("/resource/getSubIndustry",parameters: {"id":id});
      if(DioUtil.checkRequestResult(value)){
        if(value["data"]!=null&&value["data"]!=""){
          mListOne.add(value["data"] as List);
          level++;
  ...
  search(String value){
    if(isGangWei){
      DioUtil.request("/resource/searchPosition",parameters: {"name":value}).then((value){
        if(value["data"]!=null&&value["data"]!=""){
          mListOne.add(value["data"] as List);
          level++;
          notifyListeners();
      ...
      DioUtil.request("/resource/searchIndustry",parameters: {"name":value}).then((value){
        if(value["data"]!=null&&value["data"]!=""){
          mListOne.add(value["data"] as List);
          level++;
          notifyListeners();
...
class ChooseWorkName extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    return BaseView(
      ...
      onReady: (model){model.isGangWei = isGangWeis;model.init();},
        ...
        return WillPopScope(child:Scaffold(
          ...
          appBar: PreferredSize(
              child: AppBar(
                ...
                title: Text(isGangWeis?"选择岗位类型":"选择行业",style: TextStyle(fontWeight: FontWeight.bold,fontSize: 17.0,color: Color.fromRGBO(51, 51, 51, 1)),),
                centerTitle: true,
              ),
              ...
              children: <Widget>[
                Container(
                  ...
                  child: Row(
                    ...
                    children: <Widget>[
                      Icon(Icons.search,size: 15.0, color: Color.fromRGBO(1, 1, 1, 0.5)),
                      SizedBox(width: 10.0,),
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            hintText: "搜 索",
                            hintStyle: TextStyle(color: Color.fromRGBO(1, 1, 1, 0.5),fontSize: 13.0),
                ...
                Expanded(child: ListView(
                  children: _getList(context,model)
                )),
            ],
          ),)
        ),
  ...
  Widget _getItem(BuildContext context,ChooseWorkNameProvider model,Map map){
    ...
    return Center(child:GestureDetector(child:Container(height: 50.0,width: 360,
      decoration: BoxDecoration(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          flagShow ? Image.asset(join(AssetsUtil.assetsDirectoryCommon,"icon_forward_normal.png"),width: 5.0,) : SizedBox(width: 5.0,) ,
        ],
      )),
      onTap: (){
        //点击item
