...
class MarketPostList extends StatefulWidget {
  ...
  MarketPostList(this.postList, this.apiUrl, this.params,
      {this.refreshByMine = false,
      this.controller,
      this.marketPostListProvider,
      this.scrollerFlag = false,
      this.darkModel = false});

  @override
  _MarketPostListState createState() =>
      _MarketPostListState(marketPostListProvider);
}

class _MarketPostListState extends State<MarketPostList>
  ...
  Widget build(BuildContext context) {
    if ((widget.postList == null || widget.postList.length == 0) &&
        !widget.refreshByMine) {
      if (i == 0) {
        i = 1;
  ...
  Widget _buildCommentList(MarketPostListProvider model) {
    return Padding(
      ...
      child: model.mPostModel.length > 0
          ? ListView.separated(
              physics: widget.refreshByMine && !widget.scrollerFlag
                  ? NeverScrollableScrollPhysics()
                  : BouncingScrollPhysics(),
              controller: widget.refreshByMine && !widget.scrollerFlag
                  ? null
                  : scrollController,
              ...
              itemBuilder: (BuildContext context, int index) {
                ...
                return GestureDetector(
                  ...
                  onTap: () {
                    Get.to(MarketListDetail(
                            model.mPostModel[index], model.itemStuMap[index]))
                        .then((value) {
                      if (value != null) {
                        if (value == 1) {
                          model.getRefresh(widget.apiUrl, widget.params);
  ...
  // 头像
  Widget _buildRowHead(
      MarketPostModel postModel, int index, MarketPostListProvider model) {
    bool flagIsHasCircleName = false;
    if (postModel.marketCircleName != null &&
        postModel.marketCircleName != '') {
      flagIsHasCircleName = true;
    }
    return Row(
      ...
      children: <Widget>[
                ...
                child: Container(
                  ...
                  child: ClipRRect(
                    ...
                    child: CachedNetworkImage(
                        imageUrl: postModel.headPortraitUrl ??=
                    ...
                    children: <Widget>[
                      Text(postModel.name ?? "",
                          style: TextStyle(
                              ...
                              color: widget.darkModel
                                  ? Colors.white
                                  : Color.fromRGBO(1, 1, 1, 1))),
                      SizedBox(width: 5),
                      Text(postModel.companyName ?? "",
                          style: TextStyle(
                              fontSize: 13,
                  ...
                  Row(
                    children: <Widget>[
                      Text.rich(TextSpan(children: [
                        TextSpan(
                            text:
                                '${DataTimeToString.toTextString(DateTime.parse(postModel.createdTime))}发布',
                            style: TextStyle(
                                fontSize: 13,
                                color: Color.fromRGBO(141, 141, 141, 1))),
                        TextSpan(
                            text: flagIsHasCircleName ? "于" : "",
                            style: TextStyle(
                                fontSize: 13,
                                color: Color.fromRGBO(141, 141, 141, 1))),
                      ])),
                      Visibility(
                        visible: flagIsHasCircleName,
                        child: GestureDetector(
                          child: Text(postModel.marketCircleName ??= "",
                              style: TextStyle(
                                  fontSize: 13,
        ...
        postModel.ucoinAmount != 0.0
            ? Column(
                crossAxisAlignment: CrossAxisAlignment.center,
                children: <Widget>[
                  Image.asset(join(
                      AssetsUtil.assetsDirectoryMarket, 'market_coin.png')),
                  Divider(color: Colors.transparent, height: 2),
                  Text('${postModel.ucoinAmount}优币',
                      style: TextStyle(
                          fontSize: 10, color: Color.fromRGBO(255, 221, 61, 1)))
                ],
              )
            : SizedBox.shrink(),
      ],
  ...
  Widget _buildContent(
    ...
    spans.add(TextSpan(
        ...
        style: TextStyle(
            fontSize: 15,
            color:
                widget.darkModel ? Colors.white : Color.fromRGBO(1, 1, 1, 1))));
        ...
        return Container(
            ...
            child: GestureDetector(
              ...
              child: Container(
                ...
                child: Stack(
                  children: <Widget>[
                    UiUtil.getHeadImage(mediaItem.coverUrl, 130.0,
                        mHeight: 220.0, isOval: false),
                    Align(
                        alignment: Alignment.center,
                        child: UiUtil.getContainer(
                            30.0,
                            15.0,
                            UiUtil.getColor(235, opacity: 0.4),
                            Icon(
                              Icons.play_arrow,
                              color: UiUtil.getColor(255, opacity: 0.6),
                            ),
                            mWidth: 30.0))
                  ],
                ),
      ...
      return GridView(
        ...
        children: mediaList.map((MarketPostMediaModel mediaModel) {
          return ClipRRect(
            ...
            child: CachedNetworkImage(
                imageUrl: mediaModel.worksUrl ??= ImProvider.DEF_HEAD_IMAGE_URL,
                fit: BoxFit.cover),
          );
        }).toList(),
      );
      ...
      children: <Widget>[
        ButtonTheme(
          ...
          child: FlatButton.icon(
              onPressed: null,
              icon: Image.asset(
                  join(AssetsUtil.assetsDirectoryMarket, 'market_share.png')),
              label: Text('${postModel.numberOfForwarding}',
                  style: TextStyle(
                      color: widget.darkModel ? Colors.white : Colors.black))),
        ),
        ButtonTheme(
          minWidth: 0,
          child: FlatButton.icon(
              onPressed: null,
              icon: Image.asset(
                  join(AssetsUtil.assetsDirectoryMarket, 'market_comment.png')),
              label: Text('${postModel.numberOfComments}',
                  style: TextStyle(
                      color: widget.darkModel ? Colors.white : Colors.black))),
        ),
        ButtonTheme(
          ...
          child: FlatButton.icon(
              ...
              icon: Image.asset(
                join(AssetsUtil.assetsDirectoryMarket, 'market_praise.png'),
                color: model.itemStuMap[index]["praiseColor"],
              ),
              label: Text('${model.itemStuMap[index]["praiseNum"]}',
                  style: TextStyle(
                      color: widget.darkModel ? Colors.white : Colors.black))),
        ),
      ],
...
class MarketPostListProvider extends ChangeNotifier {
  ...
  initProvider() {
    ...
    mPostModel.forEach((element) {
      if (mPostModel[index].isLiked == null) {
        colors = praiseColor_un;
      } else {
        colors = praiseColor_on;
      }
      Map def = {
        "praiseColor": colors,
        "praiseNum": mPostModel[index].likes ?? 0
      };
  ...
  getMorePage(String uri) {
    if (isHasNext && loadingFree) {
      ...
      params['size'] = sizeNum;
      DioUtil.request(uri, parameters: params).then((value) {
        if (DioUtil.checkRequestResult(value)) {
          if (value['data'] != null) {
            mList = (value['data'] as List).map((e) {
              return MarketPostModel.fromJson(e);
            }).toList();
            if (mList.length < sizeNum) {
              isHasNext = false;
            }
            nowPage++;
