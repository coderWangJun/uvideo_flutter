...
class CircleDetailProvider extends ChangeNotifier {
  ...
  init(num id) {
    marketId = id;
    DioUtil.request("/market/getMarketCircleDetails", parameters: {"id": id})
        .then((value) {
      if (DioUtil.checkRequestResult(value)) {
        if (value['data'] != null) {
          ...
          DioUtil.request("/market/getCircleUser", parameters: {
            "marketCircleId": id,
            "current": 1,
            "size": personNum
          }).then((value) {
  ...
  addIntoMarketCircle() {
    if (isMineCircle) {
      DioUtil.request("/market/disbandMarketCircle",
          parameters: {"marketCircleId": marketId}).then((value) {
      ...
      if (!isFlag) {
        DioUtil.request(
          "/market/addMarketCircleApplication",
          parameters: {"circleId": marketId},
        ).then((value) {
          if (DioUtil.checkRequestResult(value, showToast: true)) {
            showTitle = "已申请";
        ...
        DioUtil.request("/market/exitMarketCircle",
            parameters: {"circleId": marketId}).then((value) {
          if (DioUtil.checkRequestResult(value, showToast: true)) {
            showTitle = "申请加入";
            notifyListeners();
          }
...
class _CircleDetailState extends State<CircleDetail> {
    ...
    return BaseView(
      ...
      builder: (context, model, child) {
        return Scaffold(
                ...
                ? [
                    GestureDetector(
                        ...
                        children: <Widget>[
                          Image.asset(
                            join(AssetsUtil.assetsDirectoryCommon,
                                'icon_modify_white.png'),
                            color: UiUtil.getColor(102),
                            width: 15.0,
                            height: 15.0,
                            fit: BoxFit.cover,
                          ),
                          ...
                          Text(
                            "编辑",
                            style: UiUtil.getTextStyle(102, 15.0),
                          ),
                          SizedBox(
                            width: 10.0,
                          ),
                      ...
                      onTap: () {
                        ...
                        Get.to(MarketEdit(
                          "",
                          model.dataCircle['marketTypeId'] as num,
                          mFlagIsAdd: false,
                          marketId: id,
                        )).then((value) {
                          if (value != null) {
          ...
          body: model.isQc
              ? _getBody(context, model)
              : EmptyWidget(
                  showTitle: "该圈子因为某些原因找不了啦",
                ),
        );
      },
    );
  ...
  Widget _getBody(BuildContext context, CircleDetailProvider model) {
    int i = 0;
    return model.dataCircle.length > 0
        ? Container(
      ...
      Row(
        ...
        children: <Widget>[
          Row(
            children: <Widget>[
              UiUtil.getHeadImage(
                  map['headPortraitUrl'] ??= ImProvider.DEF_HEAD_IMAGE_URL,
                  50.0),
              SizedBox(
                width: 17.0,
              ),
                    ...
                    children: <Widget>[
                      ...
                      isMine
                          ? UiUtil.getContainer(
                              ...
                              Text(
                                "圈主",
                                style: UiUtil.getTextStyle(255, 10.0),
                              ),
...
//        aspectRatio: 16/9,
  ...
  Widget _buildImage(String imageUrl, BuildContext context) {
    return Center(
        child: UiUtil.getHeadImage(
            imageUrl, MediaQuery.of(context).size.width - 50.0,
